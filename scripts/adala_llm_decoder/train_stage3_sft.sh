#!/bin/bash
# AdaLA-style Stage 3 for the NVILA LLM decoder.
# This is an additive wrapper around the existing linearized decoder SFT path.
...
CUSTOM_YAML=$(mktemp /tmp/adala_llm_stage3_datasets_XXXXXX.yaml)
trap "rm -f ${CUSTOM_YAML} ${TEST_JSONL:-}" EXIT

echo "---" > "${CUSTOM_YAML}"

if [ "${TEST_MODE}" = "1" ]; then
    TEST_JSONL=$(mktemp /tmp/adala_llm_stage3_test_XXXXXX.jsonl)
...
RUN_NAME=${RUN_NAME:-"adala-llm-stage3-sft-${ATTENTION_TYPE}"}
...
echo "STAGE2_CHECKPOINT   = ${STAGE2_CHECKPOINT_PATH}"
echo "ATTENTION_ONLY_CKPT = ${STAGE1_CHECKPOINT_PATH}"
