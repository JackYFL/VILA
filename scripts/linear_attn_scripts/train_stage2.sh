#!/bin/bash
# Stage2: SFT with vanilla (non-gated) linear attention (VanillaLinearAttention) + LoRA.
# Replaces all softmax self-attn with VanillaLinearAttention (trainable feature_map_q/k),
# then fine-tunes with LoRA using standard token-prediction loss.
# Run from the VILA root directory: bash scripts/linear_attn_scripts/train_stage2.sh
#
# Unlike the Lizard variant, vanilla linear attention trains from the base NVILA
# in a single stage2 run — no stage1 distillation checkpoint is required.
# Set STAGE1_CHECKPOINT_PATH if you later want to warm-start from a stage1 run.
...
# ============================================================
DEFAULT_RUN_NAME="nvila-8b-llava-onevision-img-linear-attn-stage2"
...
# Base model (same as used in lizard stage1)
MODEL_PATH=${MODEL_PATH:-"Efficient-Large-Model/NVILA-8B"}

# Optional stage1 checkpoint — leave empty for vanilla (no warm-start needed).
...
# Usage: TEST_MODE=1 bash scripts/linear_attn_scripts/train_stage2.sh
# ============================================================
...
echo "STAGE1_CHECKPOINT    = ${STAGE1_CHECKPOINT_PATH:-<none>}"
echo "ATTENTION_TYPE       = linear_attn"
...
if [ -n "${STAGE1_CHECKPOINT_PATH}" ]; then
    STAGE1_FLAG="--stage1_checkpoint_path ${STAGE1_CHECKPOINT_PATH}"
fi
